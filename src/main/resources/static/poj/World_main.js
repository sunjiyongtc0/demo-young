...
// 路径配置begin
// 路径配置end
world.defaultcolor=['#3cc444','#ffb500','#e32037'];//绿色,黄色,红色
/***************************************地图初始化方法begin***************************************/
world.initMap=function(options){
	var nameMap = {
			 'Afghanistan':' ',
               'Angola':' ',
               'Albania':' ',
		...
		var config={
		    ...
		    "formatter":function(option){
		    	 return option.param.name;
		    }
		};
	    $.extend(config,options);
	    $.extend(nameMap,config.nameMap);
	    var dom_1;// 创建dom模型对象
	    if(typeof config.selector=="string"){
	    	if($("#"+config.selector).length==0){
	    		console.error("容器对象不存在");
	    		return;
	    	}
	    	dom_1=$("#"+config.selector)[0];
	    }
	    if(typeof config.selector=="object"){
	    	...
	    	dom_1=config.selector.length>0 ? config.selector[0]:config.selector;
	    }
				...
				tooltip : {
					trigger : 'item',
					 formatter: function (param,ticket,callback) {
						  return config.formatter({config:config,param:param,ticket:ticket,callback:callback});
				     }
				},
				series : [ {
					name : 'china',
					...
					nameMap : nameMap,
		            geoCoord:{//特殊处理广东、上海用户的坐标显示
		            	'阿富汗':[67.709953,33.93911],
						'印度尼西亚':[114.727669,4.535277],
		            	...
		            	'新加坡':[103.70693,1.3147268],
		            	'香港':[113.8412347,22.3574398]
                    }
				}]
		};
		myChart.setOption(option);// 为echarts对象加载数据
		
		myChart.on(echarts.config.EVENT.DBLCLICK,function(param){
			config.dblclick({myChart:myChart,target:dom_1,param:param});
		});
		myChart.on(echarts.config.EVENT.CLICK,function(param){
			config.click({myChart:myChart,target:dom_1,param:param});
		});
   
}
/***************************************地图初始化方法end***************************************/
/***************************************柱状图初始化方法begin***************************************/
world.initBar=function(options){
	var config={
			"selector":"div_bar",// 选择器ID
			"text":"",
		    "subtext" : "",
		    "data_1":['安徽', '河北', '湖南', '贵州', '北京'],
	        "data_2":[26,35,58,62,68],
	        "color_1": [ '#219b28','#3db188','#f7b81d','#f5692d','#e32037'],
	        "grid":{x:'50',y:70, y2:30, x2:'2%'},
	        "name_1":""
    };
    $.extend(config,options);
    //require 命令begin
			            ...
			            itemStyle: {
			                normal: {
			                    color: function(params) {
			                        return config.color_1[config.color_1.length-1-params.dataIndex]
			                    },
			                    label: {
			                        show: true,
			                        position: 'top',
...
/***************************************柱状图初始化方法end***************************************/
acsno.world=world;
})((window.acsno=window.acsno||{}).customer||{},jQuery);